...
public class EnumDropDownUtilities {
    ...
    private void fillDropDown(final String value,
        ...
        for (int i = 0; i < enumeratedValues.length; i++) {
            ...
            if (enumeratedValue.indexOf('=') > 0) {
                //using a mapping
                final String[] splut = ConstraintValueHelper.splitValue(enumeratedValue);
                final String realValue = splut[0];
                final String display = splut[1];
                val = realValue;
    ...
    private Set<String> getCurrentValues(final String value,
        ...
        if (isMultipleSelect && value != null) {
            final String[] split = ListSplitter.split("\"",
                                                      true,
                                                      removeBrackets(value));

            for (String val : Arrays.asList(split)) {
                result.add(val.trim());
            }
        } else {
            result.add(value);
